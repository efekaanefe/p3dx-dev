...
class ArucoDetector(Node):
    def __init__(self, simulation=True):
        ...
        if simulation:
            ...
            self.publisher = self.create_publisher(Twist, '/aruco_vel', 10)
    ...
    def check_location(self):
        if self.last_seen_corners is None:
            ...
            if closeness_to_target > BACK_UP_THRESHOLD:
                self.get_logger().info("Too close, backing up..")
            elif MOVE_THRESHOLD < closeness_to_target < BACK_UP_THRESHOLD:
                self.linear_vel = 0
                self.get_logger().info("Staying here..")
                return
            else:
...
def main():
    ...
    try:
        ...
        cv2.destroyAllWindows()
        detector.get_logger().info("Shutting down ArUco Detector node.")
    finally:
